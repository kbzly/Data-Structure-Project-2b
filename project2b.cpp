...
void testHiddenData() {
    ...
    eecsPQ.push(HiddenData{10});
    eecsPQ.push(HiddenData{25});
    eecsPQ.push(HiddenData{15});

    assert(eecsPQ.size() == 3);
    assert(eecsPQ.top().data == 25); 

    eecsPQ.pop();
    assert(eecsPQ.size() == 2);
    assert(eecsPQ.top().data == 15);

    eecsPQ.push(HiddenData{5});
    assert(eecsPQ.size() == 3);
    assert(eecsPQ.top().data == 15);
...
void testUpdatePriorities() {
    ...
    PQ<const int *, IntPtrComp> pq0 {};
    Eecs281PQ<const int *, IntPtrComp> &eecsPQ0 = pq0;

    for (auto &datum0 : test0) {
        eecsPQ0.push(&datum0);
    }

    auto &datum0 = test0[5];
    datum0 = 100;
    eecsPQ0.updatePriorities();
    assert(*eecsPQ0.top() == 100);
    assert(eecsPQ0.top() == &datum0);
...
void testPairing() {
...

    {
        ...
        // Initialize pairing heap with a range-based constructor
        PairingPQ<int> pairingHeap { inputValues.cbegin(), inputValues.cend() };

        // Add new nodes and store the pointers to these nodes
        PairingPQ<int>::Node* node50 = pairingHeap.addNode(50);
        PairingPQ<int>::Node* node70 = pairingHeap.addNode(70);

        assert(pairingHeap.top() == 70);

        pairingHeap.updateElt(node70, 80);
        assert(pairingHeap.top() == 80);

        pairingHeap.updateElt(node50, 15);
        assert(pairingHeap.top() == 80);

        pairingHeap.pop();
        assert(pairingHeap.top() == 60);

        pairingHeap.pop();
        assert(pairingHeap.top() == 50);
